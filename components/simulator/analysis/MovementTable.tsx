    ...
    <div className="card overflow-x-auto">
      ...
      >
        <h3 className="text-lg font-semibold text-tac-accent">
          What moves &amp; why{" "}
          <span className="text-xs font-normal text-tac-muted">
            ({movement.length} orders, {movedCount} shifted)
          </span>
        </h3>
        <span className="text-tac-muted text-sm">{open ? "▾ Hide" : "▸ Show"}</span>
      </button>

      {/* Heading shown only in the PDF (the toggle button is hidden there) */}
      <h3 className="hidden print:block text-lg font-semibold text-tac-accent mb-2">
        What moves &amp; why
      </h3>

      {/* Collapsed on screen, always expanded in the PDF */}
      <div className={open ? "mt-4" : "hidden print:block"}>
